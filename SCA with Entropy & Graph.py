...
def calculate_guessing_entropy(predictions, true_values, num_keys=256):
    ...
    for i in range(predictions.shape[0]):
        ...
        correct_key_rank = np.where(sorted_predictions == true_values[i])[0][0]
        guessing_entropy[i] = correct_key_rank
    return np.mean(guessing_entropy)

# Plot Guessing Entropy
guessing_entropies = []
for i in range(1, len(X_attack) + 1):
    subset_predictions = model.predict(X_attack[:i])
    ge = calculate_guessing_entropy(subset_predictions, y_attack[:i])
    guessing_entropies.append(ge)
    if i % 100 == 0:
        print(f"Processed {i} traces, current guessing entropy: {ge}")

plt.figure(figsize=(10, 6))
plt.plot(guessing_entropies, label='Guessing Entropy')
plt.xlabel('Number of Attack Traces')
plt.ylabel('Guessing Entropy')
plt.title('Guessing Entropy vs. Number of Attack Traces')
plt.legend()
plt.grid(True)
plt.show()
